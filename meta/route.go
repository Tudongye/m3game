...
func (r RouteApp) Parse() (env string, world string, fun string, ins string, err error) {
	...
	groups := regexAppID.FindStringSubmatch(string(r))
	if len(groups) == 0 {
		err = errs.MetaRouteAppParseFail.New("RouteApp Parse fail %s", string(r))
	} else {
		env = groups[1]
		world = groups[2]
		fun = groups[3]
...
func (r RouteSvc) Parse() (env string, world string, fun string, err error) {
	...
	groups := regexSvcID.FindStringSubmatch(string(r))
	if len(groups) == 0 {
		err = errs.MetaRouteSvcParseFail.New("RouteSvc Parse fail %s", string(r))
	} else {
		env = groups[1]
		world = groups[2]
		fun = groups[3]
...
func (r RouteWorld) Parse() (env string, world string, err error) {
	...
	groups := regexWorldID.FindStringSubmatch(string(r))
	if len(groups) == 0 {
		err = errs.MetaRouteWorldParseFail.New("RouteWorld Parse fail %s", string(r))
	} else {
		env = groups[1]
		world = groups[2]
	}
...
func (r RouteEnv) Parse() (env string, err error) {
	env = string(r)
	return
}
