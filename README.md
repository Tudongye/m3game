...
Multi，Async，Actor-Server: 游戏后台常见的业务模式，分别对应并发，单线程异步，Actor模式
...
example/dirapp 是一个无状态的并发服务，该服务提供Hello的RPC接口，其在业务层包含一个App 和 一个MultiServer
...
func New() *DirSer {
	return &DirSer{
		Server: multi.New("DirSer"),  // Multi，并发服务
	}
}
...
func (d *DirApp) Alive(app string, svc string) bool {
	return true
}
...
func Run() error {
	...
	runtime.New().Run(newApp(), []server.Server{dirserver.New()})   // 逻辑注入到框架层运行
	return nil
}
```
...
游戏后台服务常见的业务模型有 Multi 多线程，Async 单线程异步，Actor 模式 三种（暂时没见过更复杂的模型）

### Multi

Multi 多线程模型，主要用于无状态服务，M3采用原生Grpc服务实现。
...
当前支持 P2P,Random,Hash,BroadCast,MultiCast,Single路由模式
...
| MultiCast  | 目标TopicID | 对订阅目标TopciID的所有实例广播 |
| Single  | 目标服务ID | 对目标服务中ID最小的实例寻路 |

### 广播

M3基于Broker插件，实现了GrpcSer兼容的BrokerSer，用于处理BroadCast和MultiCast等单向Notify式RPC调用。
