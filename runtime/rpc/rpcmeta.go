...
func (r *RPCMeta) HashKey(msg proto.Message) (string, error) {
	...
	default:
		return "", errs.RPCCantFindHashKey.New("Unknow HashKey Kind %v", r.hashkeyd.Kind())
	}
	return hashkey, nil
}
...
func InjectionRPC(serviced protoreflect.ServiceDescriptor) error {
	for i := 0; i < serviced.Methods().Len(); i++ {
		...
		if v := proto.GetExtension(rpcde.Options(), metapb.E_RpcOption); v == nil {
			return errs.RPCInJectFail.New("RPC %s not have E_RpcOption", rpcname)
		} else if m3grpcopt, ok := v.(*metapb.M3RpcOption); !ok {
			return errs.RPCInJectFail.New("RPC %s E_RpcOption type err", rpcname)
		} else if m3grpcopt == nil {
			return errs.RPCInJectFail.New("RPC %s E_RpcOption is nil", rpcname)
		} else {
		...
		if fieldd := inputd.Fields().ByName(protoreflect.Name(meta.grpcoption.RouteKey)); fieldd != nil {
			...
			default:
				return errs.RPCInJectFail.New("Invaild HashKey Kind %v", fieldd.Kind())
			}
		}
